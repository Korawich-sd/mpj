...
$history = $conn->prepare("SELECT * FROM history");
$history->execute();
    ...
    if (in_array($fileActExt1, $allow)) {
        if ($img['size'] > 0 && $img['error'] == 0) {
            if (move_uploaded_file($img['tmp_name'], $filePath1)) {
                $edit_history = $conn->prepare("UPDATE history SET content = :content, img = :img WHERE id = :id");
                ...
                if ($edit_history) {
                    ...
                    echo "<meta http-equiv='refresh' content='2;url=history'>";
                } else {
                    echo "<script>
                    $(document).ready(function() {
                        Swal.fire({
        ...
        $edit_history = $conn->prepare("UPDATE history SET content = :content WHERE id = :id");
        $edit_history->bindParam(":content", $content);
        $edit_history->bindParam(":id", $id);
        $edit_history->execute();
        if ($edit_history) {
