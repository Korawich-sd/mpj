...
$vision = $conn->prepare("SELECT * FROM vision");
$vision->execute();
...
if (isset($_POST['edit-vision'])) {
    ...
    $stmt = $conn->prepare("SELECT * FROM vision WHERE id = :id");
    $stmt->bindParam(":id", $id);
    $stmt->execute();
    $row_vision = $stmt->fetch(PDO::FETCH_ASSOC);


                $edit_vision = $conn->prepare("UPDATE vision SET content = :content WHERE id = :id");
                ...
                if ($edit_vision) {
                    ...
                    echo "<meta http-equiv='refresh' content='2;url=vision'>";
                } else {
                    echo "<script>
                    $(document).ready(function() {
                        Swal.fire({
