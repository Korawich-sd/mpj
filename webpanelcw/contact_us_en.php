...
$data_contact = $conn->prepare("SELECT * FROM contact_en WHERE id = :id");
    ...
    if (in_array($fileActExt1, $allow)) {
        if ($img['size'] > 0 && $img['error'] == 0) {
            if (move_uploaded_file($img['tmp_name'], $filePath1)) {
                $edit_contact = $conn->prepare("UPDATE contact_en SET company_name = :company_name, address = :address, tel1 = :tel1, tel2 = :tel2, email = :email, line = :line, line_qr = :line_qr WHERE id = :id");
        ...
        $edit_contact = $conn->prepare("UPDATE contact_en SET company_name = :company_name, address = :address, tel1 = :tel1, tel2 = :tel2, email = :email, line = :line WHERE id = :id");
        $edit_contact->bindParam(":company_name", $company_name);
