...
if (isset($_POST['add_email'])) {
    ...
    if (empty($email)) {
        ...
        try {
            ...
            $insert_email = $conn->prepare("INSERT INTO email_5(email,company_id)
            ...
            if ($insert_email) {
                ...
                echo "<meta http-equiv='refresh' content='1.5;url=work_with_us'>";
            } else {
                echo "<script>
                $(document).ready(function() {
                    Swal.fire({
...
if (isset($_POST['add_email_1'])) {
    ...
    if (empty($email)) {
        ...
        try {
            ...
            $insert_email = $conn->prepare("INSERT INTO email_6(email,company_id)
                                                  VALUES(:email,:company_id)");
            $insert_email->bindParam(":email", $email);
            $insert_email->bindParam(":company_id", $company_id);
            $insert_email->execute();
...
if (isset($_POST['edit_email'])) {
    ...
    $insert_email = $conn->prepare("UPDATE email_5 SET email = :email WHERE id = :id");
    $insert_email->bindParam(":email", $email);
...
if (isset($_POST['edit_email_1'])) {
    $email = $_POST['email_edit_1'];
    $id_email = $_POST['id_email_1'];

    $insert_email = $conn->prepare("UPDATE email_6 SET email = :email WHERE id = :id");
    $insert_email->bindParam(":email", $email);
    $insert_email->bindParam(":id", $id_email);
    $insert_email->execute();
...
if(isset($_POST['delete'])){
    ...
    $del_email = $conn->prepare("DELETE FROM email_5 WHERE id = :id");
...
if(isset($_POST['delete_1'])){
    $delete = $_POST['delete_1'];

    $del_email = $conn->prepare("DELETE FROM email_6 WHERE id = :id");
    $del_email->bindParam(":id",$delete);
    $del_email->execute();
...
$stmt1 = $conn->prepare("SELECT * FROM email_6");
$stmt1->execute();
$row_email_1 = $stmt1->fetchAll();
            ...
            <section class="section">
                ...
                <div class="card">
                    <div class="card-header">
                        <h4 class="card-title">Email for job application information</h4>
                        <div class="box-lang">
                    ...
                    <div class="card-header">
                        <h4 class="card-title">Email for complaints</h4>
                        <div class="box-lang">
                            <a type="input" data-bs-toggle="modal" class="btn" style="background-color: #fd7e14; color: #FFF;" href="#addactivity_1">Add</a>
                        </div>
                    </div>
                    <div class="container">
                        <div class="row">
                            <div class="col-md-12">
                                <div class="table-responsive">
                                    <table class="table">
                                        ...
                                        <tbody>
                                            <?php
                                            foreach ($row_email_1 as $row_email_1) { ?>
                                                <tr align="center">
                                                    <td><?php echo $row_email_1['email'] ?></td>
                                                    <td>
                                                        <form method="post">
                                                            <a type="input" data-bs-toggle="modal" href="#editactivity_1<?php echo $row_email_1['id'] ?>" class="btn " style="background-color:#ffc107; color: #FFFFFF;"><i class="bi bi-pencil-square"></i></a>
                                                            <button type="submit" class="btn" value="<?php echo $row_email_1['id']; ?>" onclick="return confirm('You want to delete this email?');" name="delete_1" style="background-color:#c3412c; color: #FFFFFF;"><i class="bi bi-trash"></i></button>
                                                        </form>
                                                    </td>
                                                </tr>

                                                <!-- Modal Edit email -->
                                                <div class="modal fade" id="editactivity_1<?php echo $row_email_1['id'] ?>" data-bs-backdrop="static" aria-hidden="true">
                                                    <div class="modal-dialog modal-md  modal-dialog-centered">
                                                        <div class="modal-content">
                                                            <div class="modal-header">
                                                                <h1 class="modal-title fs-5" id="staticBackdropLabel">Add Email</h1>
                                                            ...
                                                            <div class="modal-body">
                                                                <form method="post">
                                                                    <div class="row">
                                                                        <div class="col-md-12">
                                                                            <span>Email (@mpjlogistics.com)</span>
                                                                            <input type="hidden" name="id_email_1" value="<?php echo $row_email_1['id']  ?>">
                                                                            <input type="text" name="email_edit_1" value="<?php echo $row_email_1['email']  ?>" class="form-control">
                                                                        </div>
                                                                    </div>
                                                                    <div class="d-flex justify-content-center-align-item-center">
                                                                        <button type="submit" name="edit_email_1" class="btn btn-save">Save</button>
                                                                    </div>
                                                                </form>
                                                            </div>
                                                        </div>
                        ...
                        <!-- Modal Add email -->
                        <div class="modal fade" id="addactivity_1" data-bs-backdrop="static" aria-hidden="true">
                            <div class="modal-dialog modal-md  modal-dialog-centered">
                                <div class="modal-content">
                                    <div class="modal-header">
                                        <h1 class="modal-title fs-5" id="staticBackdropLabel">Add Email</h1>
                                    ...
                                    <div class="modal-body">
                                        <form method="post">
                                            ...
                                            <div class="d-flex justify-content-center-align-item-center">
                                                <button type="submit" name="add_email_1" class="btn btn-save">Save</button>
                                            </div>
                                        </form>
                                    </div>
                                </div>
