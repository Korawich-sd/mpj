...
$structure = $conn->prepare("SELECT * FROM structure_en");
$structure->execute();
...
if (isset($_POST['edit-structure'])) {
    ...
    $stmt = $conn->prepare("SELECT * FROM structure_en WHERE id = :id");
    ...
    if (in_array($fileActExt1, $allow)) {
        if ($img['size'] > 0 && $img['error'] == 0) {
            if (move_uploaded_file($img['tmp_name'], $filePath1)) {
                $edit_structure = $conn->prepare("UPDATE structure_en SET content = :content, img = :img WHERE id = :id");
                ...
                if ($edit_structure) {
                    ...
                    echo "<meta http-equiv='refresh' content='2;url=structure_en'>";
                } else {
                    echo "<script>
                    $(document).ready(function() {
                        Swal.fire({
        ...
        $edit_structure = $conn->prepare("UPDATE structure_en SET content = :content WHERE id = :id");
        $edit_structure->bindParam(":content", $content);
        $edit_structure->bindParam(":id", $id);
        $edit_structure->execute();
        if ($edit_structure) {
