...
if (isset($_POST['add_email'])) {
    ...
    if (empty($email)) {
        ...
        try {
            ...
            $insert_email = $conn->prepare("INSERT INTO email_4(email,company_id)
            ...
            if ($insert_email) {
                ...
                echo "<meta http-equiv='refresh' content='1.5;url=email_4'>";
            } else {
                echo "<script>
                $(document).ready(function() {
                    Swal.fire({
...
if (isset($_POST['edit_email'])) {
    ...
    $insert_email = $conn->prepare("UPDATE email_4 SET email = :email WHERE id = :id");
    $insert_email->bindParam(":email", $email);
...
if(isset($_POST['delete'])){
    ...
    $del_email = $conn->prepare("DELETE FROM email_4 WHERE id = :id");
...
//query email
$stmt = $conn->prepare("SELECT * FROM email_4");
$stmt->execute();
