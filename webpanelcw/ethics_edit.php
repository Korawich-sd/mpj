...
$ethics = $conn->prepare("SELECT * FROM ethics");
$ethics->execute();
...
if (isset($_GET['ethics_id'])) {
    ...
    $stmt = $conn->prepare("SELECT * FROM ethics WHERE ethics_id = :ethics_id");
...
if (isset($_POST['edit-ethics'])) {
    ...
    $text = $conn->prepare("UPDATE ethics SET content = :content");
    ...
    if ($text) {
        ...
        echo "<meta http-equiv='refresh' content='1.5;url=ethics'>";
    } else {
        echo "<script>
                    $(document).ready(function() {
                        Swal.fire({
