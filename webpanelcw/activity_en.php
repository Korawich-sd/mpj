    ...
    if (empty($topic)) {
        ...
        try {
            if (in_array($fileActExt1, $allow)) {
                if ($img['size'] > 0 && $img['error'] == 0) {
                    if (move_uploaded_file($img['tmp_name'], $filePath1)) {
                        $activity = $conn->prepare("INSERT INTO activity_en(cover_img,topic,content,date,status)
            ...
            foreach ($_FILES['img_add']['tmp_name'] as $key => $value) {
                ...
                if (in_array($extension, $supported)) {
                    ...
                    if (move_uploaded_file($_FILES['img_add']['tmp_name'][$key], "upload/upload_activity/" . $new_name)) {
                        $sql = "INSERT INTO activity_img_en (image, activity_id) VALUES(:image, :activity_id)";
            ...
            if ($activity) {
                ...
                echo "<meta http-equiv='refresh' content='1.5;url=activity_en'>";
            } else {
                echo "<script>
                $(document).ready(function() {
                    Swal.fire({
    ...
    if (in_array($fileActExt1, $allow)) {
        if ($img_edit['size'] > 0 && $img_edit['error'] == 0) {
            if (move_uploaded_file($img_edit['tmp_name'], $filePath1)) {
                $activity = $conn->prepare("UPDATE activity_en SET cover_img = :cover_img,topic = :topic,content = :content,date = :date WHERE id = :id");
                $activity->bindParam(":cover_img", $fileNew1);
                $activity->bindParam(":topic", $topic);
                $activity->bindParam(":content", $content);
                $activity->bindParam(":date", $date);
        ...
        $activity = $conn->prepare("UPDATE activity_en SET topic = :topic,content = :content,date = :date WHERE id = :id");
        $activity->bindParam(":topic", $topic);
        $activity->bindParam(":content", $content);
        $activity->bindParam(":date", $date);
        $activity->bindParam(":id", $id_act);
...
if (isset($_POST['change-status'])) {
    ...
    $stmt = $conn->prepare("UPDATE activity_en SET status = :status WHERE id =  :id");
...
if (isset($_POST['del-img'])) {
    ...
    $delete_img = $conn->prepare("DELETE FROM activity_img_en WHERE id = :id");
            ...
            <section class="section">
                ...
                <div class="card">
                    <div class="card-header">
                        ...
                        <div class="box-lang">
                            <a href="activity"><button type="button" class="btn btn-edit">TH</button></a>
